...
class ClusteredSearchModel(VectorSpaceModel):
    ...
    def _calculate_information_gain(self):
        ...
        for word in self.content_words:
            ...
            total_entropy = entropy_with + entropy_without

            # IDF値も考慮した重要度スコア
            idf = self.calculate_idf(word)
            ig = total_entropy * idf  # エントロピーとIDFの積
...
def main():
    ...
    model = ClusteredSearchModel(idf_threshold=1.0, top_k_terms=None)  # 全ての内容語を使用
